...
// YtdlpInfo is a holder of the meta data of the video that the user is trying to download
...
func main(){
	...
	if err := downloadFile(ytDlpPath, options.URL, options.AudioOnly, *config); err != nil {
...
func buildOutputFilename(template string, info *YtdlpInfo, audioOnly bool, config *Config) string {
    ...
    sanitizedTitle := strings.ReplaceAll(info.Title, ":", "-")
...
func downloadFile(ytDlpPath string, url string, audioOnly bool, config Config) error{
	...
	args = append(args, "-o", config.DefaultOutputTemplate)
